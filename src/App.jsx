...
export default function App(){
  ...
  // Left width from localStorage
  useEffect(()=>{
    const saved = localStorage.getItem("leftWidth");
    if (saved) document.documentElement.style.setProperty("--leftWidth", saved);
  }, []);

  // Build legs
  const legs = useMemo(()=>{
    const s=new Set(); rows.forEach(r=>{ const d=(r["Drivers"]??"").toString().trim(); if(d) s.add(d); });
    const list=Array.from(s).sort(); setDrivers(list); if (!selDrivers.length && list.length) setSelDrivers([list[0]]);

    const from = dateFrom ? new Date(dateFrom+"T00:00:00") : null;
    const to   = dateTo   ? new Date(dateTo+"T23:59:59")   : null;

    const mkAddr = (prefix, r) => {
      const a = r[prefix==="ship" ? "1st Shipper Address" : "Last Receiver Address"] || "";
      const c = r[prefix==="ship" ? COLS.shipperCity : COLS.receiverCity] || "";
      const s2= r[prefix==="ship" ? COLS.shipperState: COLS.receiverState] || "";
      return { full: [a,c,s2].filter(Boolean).join(", "), cityst: [c,s2].filter(Boolean).join(", ") };
    };

    const filtered = rows
      .filter(r=> selDrivers.includes((r["Drivers"]??"").toString().trim()))
      .filter(r=> !isCanceled(r[COLS.loadStatus]))
      .filter(r=>{
        const d = basis==="pickup" ? excelToDate(r["Ship Date"]) : excelToDate(r["Del. Date"]);
      ...
      .map(r=>{
        const ship = mkAddr("ship", r); const recv = mkAddr("recv", r);
        return {
          driver: (r["Drivers"]??"").toString().trim(),
          loadNo: r["Load #"],
          shipDate: excelToDate(r["Ship Date"]),
          delDate: excelToDate(r["Del. Date"]),
          origin: ship.full, originCity: ship.cityst,
          destination: recv.full, destCity: recv.cityst,
  ...
  // KPIs
  const kpi = useMemo(()=>{
    const loads=legs.length; const miles=Math.round(legs.reduce((a,b)=>a+(b.miles||0),0));
    ...
    (async()=>{
      ...
      for(let i=0;i<legs.length;i++){
        try{
          ...
          if(lg){ Ep.push({ start: lg.start_location, end: lg.end_location, color: colorByDriver(legs[i].driver) }); }
          await new Promise(r=>setTimeout(r,120));
        }catch(e){ console.warn("Directions error", e); }
      }
      if(!cancelled){ setRoutes(Rt); setEndpoints(Ep); }
    })();
    return ()=>{ cancelled=true; };
  }, [isLoaded, JSON.stringify(legs)]);

  // Map fit and dynamic height
  const mapRef = useRef(null);
  useEffect(()=>{
    const base = Math.min(78, 54 + Math.min(legs.length*4, 18)); // grow by legs but cap
    document.documentElement.style.setProperty("--mapHeight", base+"vh");
  ...
  }, [isLoaded, routes.length, endpoints.length, routeStyle, legs.length]);
  ...
  function onDown(e){ dragging.current=true; document.body.style.userSelect="none"; }
  function onMove(e){
    if(!dragging.current) return;
    const x = e.clientX; const min=320, max=800;
    const left = Math.min(max, Math.max(min, x - 16)); // padding left approx
    const px = left+"px";
    document.documentElement.style.setProperty("--leftWidth", px);
    localStorage.setItem("leftWidth", px);
  }
  function onUp(){ dragging.current=false; document.body.style.userSelect=""; }

  // expose date pickers (showPicker for better UX)
  const fromRef = useRef(null); const toRef = useRef(null);

  return (
    <div style={{ padding: 16 }} onMouseMove={onMove} onMouseUp={onUp} onMouseLeave={onUp}>
      <div className="header">
        <h1>Driver Routing Dashboard</h1>
        <div className="right">
          <button className="btn btn-accent" onClick={()=>{
            // Reset filters but keep file & rows
      ...
      <div className="topbar">
        <div className="card" style={{padding:8}}>
          <div style={{fontSize:12,color:"var(--muted)"}}>Date from</div>
          <div className="row">
            <input ref={fromRef} type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} style={{flex:1}} />
            <button className="btn" onClick={()=>fromRef.current?.showPicker?.()}>📅</button>
          </div>
        </div>
        <div className="card" style={{padding:8}}>
          <div style={{fontSize:12,color:"var(--muted)"}}>Date to</div>
          <div className="row">
            <input ref={toRef} type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)} style={{flex:1}} />
            <button className="btn" onClick={()=>toRef.current?.showPicker?.()}>📅</button>
          </div>
        </div>
        <div className="card" style={{padding:8}}>
          <div style={{fontSize:12,color:"var(--muted)"}}>Date basis</div>
        ...
        <div className="card" style={{padding:8}}>
          <div style={{fontSize:12,color:"var(--muted)"}}>Route style</div>
        ...
        <div className="card" style={{padding:8}}>
          <div style={{fontSize:12,color:"var(--muted)"}}>Traffic</div>
          <button className="btn" onClick={()=>setShowTraffic(v=>!v)}>{showTraffic? "On":"Off"}</button>
        </div>
      </div>

      <hr className="sep" />

      <div className="split" onMouseDown={(e)=>{}}>
        {/* Left column */}
        <div className="col">
          ...
          <div className="card" style={{ padding: 12 }}>
            <div className="row">
              <div className="kpi card" style={{ flex: 1 }}>
                ...
                <div className="value">{kpi.loads}</div>
              </div>
              <div className="kpi card" style={{ flex: 1 }}>
                <div className="label">Miles</div>
                <div className="value">{num(kpi.miles)}</div>
              </div>
              <div className="kpi card" style={{ flex: 1 }}>
                <div className="label">Revenue</div>
                <div className="value">{money(kpi.revenue)}</div>
              </div>
              <div className="kpi card" style={{ flex: 1 }}>
                <div className="label">Fleet RPM</div>
                <div className="value">{kpi.fleetRPM}</div>
              </div>
              <div className="kpi card" style={{ flex: 1 }}>
                <div className="label">On‑Time %</div>
                <div className="value">{kpi.ontime}%</div>
              </div>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 12 }}>
            ...
            <select multiple size={Math.min(8, Math.max(4, drivers.length))} value={selDrivers}
              onChange={(e)=> setSelDrivers([...e.target.selectedOptions].map(o=>o.value)) } style={{ width:"100%" }}>
                    ...
                    <div className="row" style={{ justifyContent:"space-between" }}>
                      <div className="row">
                        ...
                        <div className="col">
                          ...
                          <div className="load-sub">{l.shipDate? l.shipDate.toLocaleDateString():""} — {l.originCity} → {l.destCity}</div>
                        </div>
                      </div>
                      <div className="chips">
        ...
        <div className="card map-shell">
          {apiKey ? isLoaded ? (
            ...
            >
              ...
              {routeStyle==="driving" && routes.map((r,idx)=>(
                <DirectionsRenderer key={idx} directions={r}
                  options={{ preserveViewport:true, polylineOptions:{ strokeColor: colorByDriver(legs[idx]?.driver||String(idx)), strokeWeight:4, strokeOpacity:.95 } }} />
              ))}

              {routeStyle==="lines" && endpoints.map((ep,idx)=>{
                const mid = new google.maps.LatLng( (ep.start.lat()+ep.end.lat())/2, (ep.start.lng()+ep.end.lng())/2 );
                return (
                  <React.Fragment key={idx}>
                    ...
                    {/* number badge near lane */}
                    <Marker position={mid} label={{ text: String(idx+1), color: "#0b0d12", fontWeight: "700" }}
          ...
          <div className="legend">Route style: {routeStyle==="lines" ? "Straight lines" : "Driving directions"} • Traffic: {showTraffic? "On":"Off"}</div>
        </div>
      </div>
    </div>
  );
