...
import { useDashboardFilters } from '@/state/filters';   // existing (basis, dateRange, selectedDriverIds, selectedTruckIds, filterMode)
...
export function useFinance(timegrain: 'day'|'week'|'month' = 'day'): {
  ...
  const { basis, dateRange, selectedDriverIds, selectedTruckIds, filterMode } = useDashboardFilters();
  ...
  return useMemo(()=>{
    ...
    const { byTruck, fleetTotals } = buildFinance(loads, fuel, exps, {
      ...
      trucks: filterMode === 'truck' ? (selectedTruckIds || []) : [],
      drivers: filterMode === 'driver' ? (selectedDriverIds || []) : [],
    }, timegrain);

    return { rows: byTruck, fleet: fleetTotals };
  }, [loadsRaw, fuelRaw, expRaw, basis, dateRange.start, dateRange.end, selectedDriverIds?.join('|'), selectedTruckIds?.join('|'), timegrain, filterMode]);
}
